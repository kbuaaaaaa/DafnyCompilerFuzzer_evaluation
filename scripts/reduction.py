...
async def reduction(processing=False, output_dir=None, language=None, interpret=False):
    ...
    try:
        ...
        process = await asyncio.create_subprocess_shell("creduce --not-c --no-default-passes --add-pass pass_lines 1 0 " + f"{output_dir}creduce-{language}/{language}-interestingness_test.sh " + f"{output_dir}creduce-{language}/main.dfy", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
