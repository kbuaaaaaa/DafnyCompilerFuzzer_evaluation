...
if __name__ == "__main__":
    ...
    while (time.time() - start_time) < duration:
        ...
        if output.returncode == 0:
            ...
            subprocess.run(["aws", "s3", "cp", f"{output_dir}/main.dfy", f"s3://compfuzzci/evaluation/all_test/{branch_commit}/{time}/{repetition}/{output_dir}/main.dfy"], check=True)
            subprocess.run(["aws", "s3", "cp", f"{output_dir}/fuzz-d.log", f"s3://compfuzzci/evaluation/all_test/{branch_commit}/{time}/{repetition}/{output_dir}/fuzz-d.log"], check=True)
